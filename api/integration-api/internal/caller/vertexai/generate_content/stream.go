...
func NewStream(logger commons.Logger, credential *protos.Credential) (internal_callers.ChatStream, error) {
	if _, _, _, err := internal_vertexai_common.ResolveCredential(credential); err != nil {
		logger.Errorf("Failed to create VertexAI generate_content stream client: %v", err)
		return nil, err
	}
	return &streamCaller{
		logger:     logger,
...
func (s *streamCaller) Chat(
	...
	client, err := internal_vertexai_common.NewClient(s.credential)
	if err != nil {
		if onError != nil {
			onError(requestID, err)
		}
	...
	hasToolCalls := false
	accumulator := &vertexAIChatCompletionAccumulator{}
		...
		for _, choice := range resp.Candidates {
			...
			for _, part := range choice.Content.Parts {
				if part.FunctionCall != nil {
					...
					if marshalErr != nil {
						s.logger.Warnf("failed to marshal vertexai function args: %v", marshalErr)
						argsJSON = []byte("{}")
					}
	...
	metrics.OnAddMetrics(internal_vertexai_common.UsageMetrics(accumulator.UsageMetadata)...)
