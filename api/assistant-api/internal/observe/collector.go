...
func NewEventCollector(logger commons.Logger, _ SessionMeta, exporters ...EventExporter) EventCollector {
	return telemetry.NewEventCollector(logger, exporters...)
}

// NewMetricCollector returns a fan-out MetricCollector. When no exporters are
// provided, a no-op implementation is returned to avoid any allocations.
func NewMetricCollector(logger commons.Logger, _ SessionMeta, exporters ...MetricExporter) MetricCollector {
	return telemetry.NewMetricCollector(logger, exporters...)
}
