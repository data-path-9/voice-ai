...
func (resampler *telnyxFakeResampler) Resample(_ []byte, _, _ *protos.AudioConfig) ([]byte, error) {
...
func TestAudioProcessor_ProcessProviderAudioFrame_EmitsBridgeAndThresholdedPipelineAudio(t *testing.T) {
	...
	_, _ = audioProcessor.ProcessProviderAudioFrame(internal_telephony_media.ProviderAudioFrame{Audio: []byte{2}})
	thirdFrame, err := audioProcessor.ProcessProviderAudioFrame(internal_telephony_media.ProviderAudioFrame{Audio: []byte{3}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
